 ...
 * @link        https://github.com/ControlledChaos/amcd-plugin
 * @license     GPL-3.0+ http://www.gnu.org/licenses/gpl-3.0.txt
 *
 * Plugin Name:  Alana Morshead Website
 * Plugin URI:   https://github.com/ControlledChaos/amcd-plugin
...
final class Alana_Morshead {
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, __( 'This is not allowed.', 'amcd-plugin' ), '1.0.0' );
	...
	private function constants() {
		 ...
		 */
		if ( ! defined( 'AMCD_VERSION' ) ) {
			define( 'AMCD_VERSION', '1.0.0' );
		}
		 ...
		 */
		if ( ! defined( 'AMCD_PATH' ) ) {
			define( 'AMCD_PATH', plugin_dir_path( __FILE__ ) );
		}
		 ...
		 */
		if ( ! defined( 'AMCD_URL' ) ) {
			define( 'AMCD_URL', plugin_dir_url( __FILE__ ) );
		}
		 ...
		 * Change the second part of the define(), here as 'amcd-plugin',
		 * to your preferred page slug.
		 *
		 * @since  1.0.0
		 * @return string Returns the URL slug of the admin pages.
		 */
		if ( ! defined( 'AMCD_ADMIN_SLUG' ) ) {
			define( 'AMCD_ADMIN_SLUG', 'amcd-plugin' );
		}
		 ...
		 */
		if ( ! defined( 'AMCD_DEFAULT_META_IMAGE' ) ) {
			define(
				'AMCD_DEFAULT_META_IMAGE',
	...
	private function dependencies() {
		...
		require_once AMCD_PATH . 'includes/class-init.php';

		// Include the activation class.
		require_once AMCD_PATH . 'includes/class-activate.php';

		// Include the deactivation class.
		require_once AMCD_PATH . 'includes/class-deactivate.php';
 ...
 * @since  1.0.0
 * @access public
 * @return object Returns the instance of the `Alana_Morshead` class.
 */
function amcd_plugin() {

	return Alana_Morshead::instance();
 ...
 * @return void
 */
register_activation_hook( __FILE__, '\amcd_activate_plugin' );
register_deactivation_hook( __FILE__, '\amcd_deactivate_plugin' );
 ...
 * @return void
 */
function amcd_deactivate_plugin() {
 ...
 */
function amcd_about_link( $links ) {
	...
	if ( is_admin() ) {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			...
			// Get the field.
			$acf_position = get_field( 'amcd_site_plugin_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'amcd_site_plugin_link_position' );
		}

		if ( true == $position ) {
			$url = admin_url( 'index.php?page=' . AMCD_ADMIN_SLUG . '-settings' );
		} else {
			$url = admin_url( 'admin.php?page=' . AMCD_ADMIN_SLUG . '-settings' );
		}

		// Create new settings link array as a variable.
		$about_page = [
			sprintf(
				'<a href="%1s" class="' . AMCD_ADMIN_SLUG . '-page-link">%2s</a>',
				admin_url( 'plugins.php?page=' . AMCD_ADMIN_SLUG . '-page' ),
				esc_attr( 'Documentation', 'amcd-plugin' )
			),
		];
...
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'amcd_about_link' );
 ...
 * @link   https://codex.wordpress.org/Plugin_API/Filter_Reference/plugin_action_links_(plugin_file_name)
 */
function amcd_settings_links( $links, $file ) {
	...
	if ( is_admin() ) {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			...
			// Get the field.
			$acf_position = get_field( 'amcd_settings_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'amcd_site_settings_position' );
		}
		...
		if ( $file == plugin_basename( __FILE__ ) ) {
			...
			$links[] = sprintf(
				'<a href="%1s" class="' . AMCD_ADMIN_SLUG . '-settings-link">%2s</a>',
				$url,
				esc_attr( 'Site Settings', 'amcd-plugin' )
			);
			$links[] = sprintf(
				'<a href="%1s" class="' . AMCD_ADMIN_SLUG . '-scripts-link">%2s</a>',
				admin_url( 'options-general.php?page=' . AMCD_ADMIN_SLUG . '-scripts' ),
				esc_attr( 'Script Options', 'amcd-plugin' )
			);
...
add_filter( 'plugin_row_meta', 'amcd_settings_links', 10, 2 );