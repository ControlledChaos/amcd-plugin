...
class Meta_Description {
	...
	public function __construct() {
		...
		add_action( 'amcd_meta_description_tag', [ $this, 'description' ] );
	...
	public function description() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			 ...
			 */
			$acf_blog_desc = get_field( 'amcd_meta_blog_description', 'option' );
			...
			$wp_blog_desc = get_option( 'amcd_meta_blog_description' );
		...
		$search_desc      = wp_strip_all_tags(
			sprintf(
				'%1s \'%2s\'',
				__( 'Showing results for', 'amcd-plugin' ),
				$search_query
			)
		);

		// Apply a filter to hard-coded text.
		$search_meta_desc = apply_filters( 'amcd_meta_description_search', $search_desc );
		...
		$meta_description = apply_filters( 'amcd_meta_description', $description );
 ...
 */
function amcd_meta_description() {
