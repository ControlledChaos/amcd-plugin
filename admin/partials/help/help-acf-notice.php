...
<div>
	<h3><?php _e( 'Advanced Custom Fields Notice', 'amcd-plugin' ); ?></h3>
	<?php echo sprintf(
		...
		esc_html__( 'The tabs and the settings on this page are reproduced by different code within this plugin when the', 'amcd-plugin' ),
		esc_html__( 'plugin is installed and activated. Specifically it needs to be the', 'amcd-plugin' ),
		esc_url( 'https://www.advancedcustomfields.com/pro' ),
		esc_html__( 'Pro edition', 'amcd-plugin' ),
		esc_html__( 'of the plugin or the', 'amcd-plugin' ),
		esc_url( 'https://wordpress.org/plugins/advanced-custom-fields/' ),
		esc_html__( 'free version', 'amcd-plugin' ),
		esc_html__( 'plus the', 'amcd-plugin' ),
		esc_url( 'https://www.advancedcustomfields.com/add-ons/options-page/' ),
		esc_html__( 'Options Page addon.', 'amcd-plugin' )
	); ?>
	<p><?php _e( 'One of the advantages of using ACF with the settings page is that the settings can be more easily added, editted, and rearranged than recoding this plugin using the WordPress Settings API. It is far easier to implement complicated settings fields, and has a nicer user interface than the native WordPress settings fields.', 'amcd-plugin' ); ?></p>
	<p><?php _e( 'This plugin comes equipped with an ACF fields import tool that will import the ACF fields registered by this plugin so that they can be modified.', 'amcd-plugin' ); ?></p>
</div>