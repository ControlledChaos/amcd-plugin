...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'General', 'amcd-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Vendor', 'amcd-plugin' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'amcd_tabs_script_options', $tabs );
...
if ( 'general' == $active_tab ) {
    $section = 'amcd-scripts-general';
    $fields  = 'amcd-scripts-general';
} elseif ( 'vendor' == $active_tab ) {
    $section = 'amcd-scripts-vendor';
    $fields  = 'amcd-scripts-vendor';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'amcd_section_script_options', $section );
$do_fields  = apply_filters( 'amcd_fields_script_options', $fields );
 ...
 */
if ( 'general' == $active_tab  ) {
    $save = __( 'Save General', 'amcd-plugin' );
} elseif ( 'vendor' == $active_tab ) {
    $save = __( 'Save Vendor', 'amcd-plugin' );
} else {
    $save = __( 'Save Settings', 'amcd-plugin' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'amcd_save_script_options', $save );

?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e( 'Script Options', 'amcd-plugin' ); ?></h1>
    <?php if ( is_rtl() ) : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper left.', 'amcd-plugin' ); ?></p>
    <?php else : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper right.', 'amcd-plugin' ); ?></p>
    <?php endif; ?>
