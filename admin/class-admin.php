...
class Admin {
	...
	private function dependencies() {
		...
		require_once AMCD_PATH . 'admin/class-settings.php';

		// Add icons to the titles of ACF tab and accordion fields, if active.
		if ( class_exists( 'acf_pro' ) && ! get_option( 'amcd_acf_activate_settings_page' ) ) {
			include_once AMCD_PATH . 'admin/class-acf-tab-icons.php';
		}

		// Include custom fields for Advanced Custom Fields Pro, if active.
		if ( class_exists( 'acf_pro' ) && ! get_option( 'amcd_acf_activate_settings_page' ) ) {
			include_once AMCD_PATH . 'admin/class-settings-fields-site-acf.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once AMCD_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once AMCD_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once AMCD_PATH . 'admin/class-admin-toolbar-menus.php';

		// Functions for various admin pages and edit screens.
		require_once AMCD_PATH . 'admin/class-admin-pages.php';

		// Import custom fields for editing, if ACF Pro is active.
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			include_once AMCD_PATH . 'admin/class-fields-import.php';
		}

		// Filter by page template.
		require_once AMCD_PATH . 'admin/class-admin-template-filter.php';
	...
	public function admin_footer() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			 ...
			 */
			$credit = get_field( 'amcd_admin_footer_credit', 'option' );
			$link   = get_field( 'amcd_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'amcd-plugin' ),
					esc_url( $link ),
					$credit
				);
			...
			// If no input we use the name of the site.
				...
				$footer = sprintf(
					'%1s %2s. ',
					$site,
					esc_html__( 'website powered by WordPress', 'amcd-plugin' )
				);
			}
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'amcd_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'amcd_footer_link' ) );
		...
		$admin_footer = apply_filters( 'amcd_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( AMCD_ADMIN_SLUG . '-admin', AMCD_URL . 'admin/assets/css/admin.min.css', [], AMCD_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( AMCD_ADMIN_SLUG . '-tooltips', AMCD_URL . 'admin/assets/css/tooltips.min.css', [], AMCD_VERSION, 'all' );
		 ...
		 */
		if ( class_exists( 'acf' ) ) {
			wp_enqueue_style( AMCD_ADMIN_SLUG . '-acf', AMCD_URL . 'admin/assets/css/acf.css', [], AMCD_VERSION, 'all' );
		}
		...
		if ( $welcome ) {
			wp_enqueue_style( AMCD_ADMIN_SLUG . '-welcome', AMCD_URL . 'admin/assets/css/welcome.css', [], AMCD_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require AMCD_PATH . 'partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( AMCD_ADMIN_SLUG . '-conditionalize', AMCD_URL . 'admin/assets/js/admin.js', [ 'jquery' ], AMCD_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( AMCD_ADMIN_SLUG . '-admin', AMCD_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], AMCD_VERSION, true );
