...
class Admin_Pages {
    ...
    public function about_plugin() {
        ...
        if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
            ...
            // Get the field.
            $link_label = get_field( 'amcd_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'amcd_site_plugin_link_label' ) );
        }
        ...
        // Otherwise use Site Plugin as the label.
            ...
            $label = __( 'Site Plugin', 'amcd-plugin' );
        }
        ...
        if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'amcd_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'amcd_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'amcd-plugin' );
        }
    ...
    public function about_plugin_output() {
        ...
        require AMCD_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'amcd-plugin' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'amcd-plugin' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once AMCD_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once AMCD_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'amcd-plugin' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'amcd-plugin' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            '<p>%1s <br /><a href="%2s" target="_blank">%3s</a> <br />%4s</p>',
            __( 'Visit:', 'amcd-plugin' ),
            'http://ccdzine.com/',
            'Controlled Chaos Design',
            __( 'for more free downloads.', 'amcd-plugin' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'amcd-plugin' )
         );
    ...
    public function title_placeholders( $title ) {
        ...
        if ( 'post' == $screen->post_type ) {
            $post_title = esc_html__( 'Post Title', 'amcd-plugin' );

        // Post type: page.
        } elseif ( 'page' == $screen->post_type ) {
            $post_title = esc_html__( 'Page Title', 'amcd-plugin' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'amcd-plugin' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'amcd_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'amcd_column_thumbnail_size', $size );
    ...
    public function image_column_head( $defaults ) {
        ...
        $name    = __( 'Featured Image', 'amcd-plugin' );

        // Apply a filter for conditional modification.
        $heading = apply_filters( 'amcd_image_column_head', $name );
